...
const COLLECTIONS = ['log', 'growth', 'memories', 'milestones', 'profile'] as const
...
async function createHousehold(env: Env): Promise<Response> {
  const id = randomCode(ID_LENGTH)
  const secret = randomCode(SECRET_LENGTH)
  const now = new Date().toISOString()
  await env.DB.prepare(
    'INSERT INTO households (id, token_hash, seq, created_at, updated_at) VALUES (?, ?, 0, ?, ?)',
  )
...
async function authenticate(
  request: Request,
  env: Env,
): Promise<{ id: string; token_hash: string; seq: number } | null> {
  ...
  const household = await env.DB.prepare('SELECT id, token_hash, seq FROM households WHERE id = ?')
    ...
    .first<{ id: string; token_hash: string; seq: number }>()
...
async function sync(request: Request, env: Env): Promise<Response> {
  ...
  let body: { cursor?: number; changes?: unknown[] }
  try {
  ...
  for (const raw of incoming) {
    ...
    if (!record) return json({ error: 'bad_record' }, 400)
    records.push(record)
  }
  ...
  if (records.length > 0) {
    ...
    const bumped = await env.DB.prepare(
      'UPDATE households SET seq = seq + 1, updated_at = ? WHERE id = ? RETURNING seq',
    )
    ...
    const statement = env.DB.prepare(
      `INSERT INTO records (household_id, collection, id, updated_at, deleted_at, data, seq)
       VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    await env.DB.batch(
      records.map((r) =>
        statement.bind(
          ...
          r.updatedAt,
          r.deletedAt ?? null,
          r.data === undefined ? null : JSON.stringify(r.data),
          seq,
        ),
      ),
    )
  ...
  const { results } = await env.DB.prepare(
    `SELECT collection, id, updated_at, deleted_at, data, seq FROM records
  ...
  const changes: WireRecord[] = (results ?? []).map((row) => ({
  ...
  const nextCursor = more ? (results![results!.length - 1].seq as number) : Math.max(cursor, seq)

  return json({ cursor: nextCursor, more, changes, serverTime: new Date().toISOString() })
}

function validateRecord(raw: unknown): WireRecord | null {
  if (typeof raw !== 'object' || raw === null) return null
  const r = raw as Record<string, unknown>
  if (typeof r.collection !== 'string' || !COLLECTIONS.includes(r.collection as Collection)) return null
  if (typeof r.id !== 'string' || r.id.length === 0 || r.id.length > 128) return null
  if (typeof r.updatedAt !== 'string' || !isIsoDate(r.updatedAt)) return null
  if (r.deletedAt != null && (typeof r.deletedAt !== 'string' || !isIsoDate(r.deletedAt))) return null
  if (r.data !== undefined && r.data !== null) {
    if (JSON.stringify(r.data).length > MAX_RECORD_BYTES) return null
  }
  return {
    collection: r.collection as Collection,
    id: r.id,
    updatedAt: r.updatedAt,
    deletedAt: (r.deletedAt as string) ?? undefined,
...
function parseAuth(header: string | null): { householdId: string; secret: string } | null {
  if (!header?.startsWith('Bearer ')) return null
  const [householdId, secret] = header.slice(7).trim().split('.')
  if (!householdId || !secret) return null
  if (householdId.length > 64 || secret.length > 128) return null
  return { householdId, secret }
}

async function tokenHash(householdId: string, secret: string): Promise<string> {
...
/** A running timer in the shared log, or the one that has just stopped */
 ...
 */
async function vapidKeys(env: Env): Promise<VapidKeys> {
  ...
  await env.DB.prepare(
    'INSERT INTO server_keys (name, value, created_at) VALUES (?, ?, ?) ON CONFLICT (name) DO NOTHING',
  )
...
async function readVapidKeys(env: Env): Promise<VapidKeys | null> {
...
async function subscribePush(request: Request, env: Env): Promise<Response> {
  ...
  await env.DB.prepare(
    `INSERT INTO push_subscriptions (household_id, endpoint, p256dh, auth, created_at, last_state)
     VALUES (?, ?, ?, ?, ?, NULL)
...
async function unsubscribePush(request: Request, env: Env): Promise<Response> {
  const household = await authenticate(request, env)
  if (!household) return json({ error: 'unauthorized' }, 401)
  ...
  if (typeof body.endpoint !== 'string') return json({ error: 'bad_subscription' }, 400)
...
async function pushTimerUpdates(env: Env): Promise<void> {
  const { results } = await env.DB.prepare(
    'SELECT household_id, endpoint, p256dh, auth, last_state FROM push_subscriptions',
  ).all<PushRow>()
  ...
  for (const row of results) {
    ...
    else byHousehold.set(row.household_id, [row])
  }
...
async function recentLogEntries(env: Env, householdId: string): Promise<Record<string, any>[]> {
...
async function babyName(env: Env, householdId: string): Promise<string | null> {
  const row = await env.DB.prepare(
    `SELECT data FROM records WHERE household_id = ? AND collection = 'profile' AND id = 'profile'`,
  )
    .bind(householdId)
    .first<{ data: string | null }>()
  if (!row?.data) return null
  try {
    const name = (JSON.parse(row.data) as { name?: unknown }).name
...
export function runningTimer(entries: Record<string, any>[], now = Date.now()): TimerState | null {
  let best: { startedAt: number; state: TimerState } | null = null
  for (const entry of entries) {
    ...
    if (entry.type === 'sleep' && typeof entry.time === 'string' && !entry.endTime) {
      ...
      if (!Number.isFinite(minutes) || minutes < 0 || minutes > MAX_SLEEP_MINUTES) continue
    ...
    if (entry.type === 'feed' && entry.kind === 'nursing' && typeof entry.sideStartedAt === 'string') {
      ...
      if (!Number.isFinite(onSide) || onSide < 0 || minutes > MAX_NURSING_MINUTES) continue
      if (!best || startedAt > best.startedAt) {
        best = {
          startedAt,
          state: {
...
export function nextMessage(
...
): { state: string; title: string; body: string } | null {
  if (running) {
  ...
  if (previous.length !== 3 || previous[2] === 'done') return null
  ...
  const sides =
    entry && numberOr(entry.leftMinutes) > 0 && numberOr(entry.rightMinutes) > 0
...
async function deliver(
  ...
  keys: VapidKeys,
  row: PushRow,
  message: { state: string; title: string; body: string },
): Promise<void> {
...
export async function vapidAuthorization(endpoint: string, keys: VapidKeys): Promise<string> {
  ...
  const signingInput = `${base64urlText(JSON.stringify({ typ: 'JWT', alg: 'ES256' }))}.${base64urlText(
    JSON.stringify(claims),
  )}`
  const key = await crypto.subtle.importKey(
    'jwk',
    { ...keys.jwk, key_ops: ['sign'] },
...
export async function encryptPushPayload(
  ...
  p256dhBase64: string,
  authBase64: string,
  material?: { salt: Uint8Array; privateJwk: JsonWebKey; publicKey: Uint8Array },
): Promise<Uint8Array> {
  ...
  const shared = new Uint8Array(
    await crypto.subtle.deriveBits({ name: 'ECDH', public: clientPublic }, serverPrivate, 256),
  )
